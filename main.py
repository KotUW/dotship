...
def link_files(file_list: List[str], CONFIG_DIR: str, forced=False):
    ...
    for index, file_path in enumerate(file_paths):
        ...
        file_dst.parent.mkdir(
            parents=True, exist_ok=True
        )  # Only fails if the directiry is a file on os.
...
def add_cmd(file_path: Path, CD: str):
    newpath = (
        CD + "/" + encode_file_path(str(file_path).replace(str(file_path.home()), ""))
    )
...
def main():
    ...
    if isAdd:
        ...
        if not add_file_path.is_absolute():
            log.error("File path has to be absolute!")
            exit(5)
        if not add_file_path.is_file():
            log.error("File path must be a file!")
            exit(6)
